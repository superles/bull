...
Job.prototype.kill = async function(signal = 'SIGKILL', timeout = 5000) {
  ...
  if(childPool){
    ...
    if(child){
      setTimeout(()=>{
        if(child.jobId === this.id){
          return killAsync(child, signal, timeout);
        }
      },0)
    }
    ...
    return new Promise(async (resolve, reject) => {
      await this.queue.isReady();
      const onCompleted = (jobId, resultValue, pid) => {
      ...
      this.queue.eclient.subscribe(this.queue.toKey('killed:success'));
      this.queue.eclient.subscribe(this.queue.toKey('killed:error'));

      const handler = (channel, message) => {
        if(/error/.test(channel)){
          removeListeners();
      ...
      this.queue.eclient.on('message', handler)


      const removeListeners = () => {
        clearInterval(interval);
        this.queue.eclient.unsubscribe(this.queue.toKey('killed:error'));
        this.queue.eclient.unsubscribe(this.queue.toKey('killed:success'));
        this.queue.eclient.off(this.queue.toKey('killed:success'), handler);
      };
      ...
      const interval = setInterval(() => {
        if (this._isQueueClosing()) {
          ...
          this.queue.getJobFromId(this.id).then(async freshJob=>{
              // let isActive = await freshJob.isActive();
              // if(!isActive){
              //   resolve();
              //   removeListeners();
              // }
      ...
      this.queue.publish('killed', JSON.stringify({ jobId: this.id, signal: signal, timeout: timeout }))
    })
