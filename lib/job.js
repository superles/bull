...
Job.prototype.kill = async function(signal = 'SIGKILL', timeout = 5000) {
   ...
   * @type ChildPool
   */
  let childPool = this.queue.childPool;

  if(childPool){
    let child = Object.values(childPool.retained).find(c=>{
      return c.jobId === this.id
    })
    if(child && child.jobId === this.id){
      return killAsync(child, signal, timeout).then(()=>{
      ...
      if (this._isQueueClosing()) {
        ...
        reject(
          new Error('cannot check if job is finished in a closing queue.')
                ...
                // this.queue.getJobFromId(this.id)
                //     .then(freshJob => {
                //         if (!freshJob || freshJob.finishedOn) {
        //   resolve();
        //   removeListeners();
        // }
        // })
