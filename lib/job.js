 ...
 */
Job.prototype.killChild = async function (child, signal = 'SIGKILL', timeout = 5000) {
  return new Promise(async (resolve, reject) => {
    ...
    if (job.killedOn && child.killed) return resolve();
    killAsync(child, signal, timeout)
        .then(res => {
          return this.queue.multi()
              .hmset(this.queue.toKey(this.id), {
                'killedOn': Date.now(),
                'killedReason': 'killed'
              })
              .exec()
        ...
        .then(() => {
          debuglog('jobId ' + this.id + ' killed via local event')
          return his.queue.publish('killed:success', {jobId: jobId, pid: child.pid});
        })
        .catch(err => {
          return this.queue.publish('killed:error', {jobId: jobId, pid: child.pid, error: err.message});
        })
        .finally(() => {
          resolve()
        })
 ...
 * @return {Promise<void>}
 */
Job.prototype.kill = async function(signal = 'SIGKILL', timeout = 5000) {
  return new Promise(async (resolve, reject) => {
    if(this.killedOn || this.killPending)  return resolve(true);
    if(this.queue.hasChild(this.id)){
      let killed = await this.queue.killJobById(this.id);
      return resolve(killed);
    }

    this.killPending = Date.now();
    this.queue.client.hset(this.queue.toKey(this.id), 'killPending' , this.killPending);

    await Promise.all([
      this.queue._registerEvent('global:killed:success'),
      this.queue._registerEvent('global:killed:error')
    ]);
        const onCompleted = (jobId, pid) => {
      if (String(jobId) === String(this.id)) {
        debuglog('jobId ' + jobId + ' killed via remote job.kill')
        resolve()
      }
    };
   const onFailed = (jobId, pid, failedReason) => {
      if (String(jobId) === String(this.id)) {
    ...
    this.queue.on('global:killed:success',onCompleted);
    this.queue.on('global:killed:error', onFailed);

    const removeListeners = () => {
      clearInterval(interval);
      clearTimeout(timeout1)
      this.queue.removeListener('global:killed:error', onFailed);
      this.queue.removeListener('global:killed:success', onCompleted);
    };
    ...
    const interval = setInterval(() => {
      if (this._isQueueClosing()) {
        ...
        Job.fromId(this.queue, this.id).then(freshJob => {
          if (freshJob.killedOn) {
            resolve();
            removeListeners();
        }
        })
    ...
    const {clientName, token} = this.data?.worker || {};

        await this.queue.publish('killed', {jobId: this.id, signal: signal, timeout: timeout, clientName, token})
...
Job.fromJSON = function(queue, json, jobId) {
  ...
  if (json.killedOn) {
    job.killedOn = parseInt(json.killedOn);
  }

  job.killPending = json.killPending;

  job.killedReason = json.killedReason;
