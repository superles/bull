...
Queue.prototype.publish = function(channel, msg) {
  ...
  return utils.isRedisReady(this.client).then(function() {
    _this.client.publish(channelKey, msg);
  });
          ...
          if(child){
            this.getJobFromId(data.jobId).then(job=>{
              killAsync(child, data.signal || "SIGKILL", data.timeout || 5000).then(()=>{
                this.publish('global:killed:success', data.jobId);
              }).catch((err)=>{
                this.publish('global:killed:error', JSON.stringify({ jobId: data.jobId, message: err.message }));
              })

            })
          }else{
            this.publish('global:killed:error', JSON.stringify({ jobId: data.jobId, message: 'process not exists' }))
          }
