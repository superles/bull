...
const {killAsync} = require('./process/utils');
 ...
 * @return {Promise<void>}
 */
Queue.prototype.publish = function(channel, msg) {
  var _this = this;
  const _eventName = channel.replace('global:', '');
  let channelKey = this.toKey(_eventName);
  msg = typeof msg === "string" ? msg : JSON.stringify(msg);
  return utils.isRedisReady(this.client).then(function() {
    return _this.client.publish(channelKey, msg);
  });
 ...
 *
 * @param jobId
 * @return {ChildProcess|Boolean
 */
Queue.prototype.hasChild = function(jobId){
  if(!this.childPool) return false;
  return Object.values(this.childPool.retained).find(c=>c.jobId === jobId);
}
Queue.prototype.killJobById = async function(jobId, signal = 'SIGKILL', timeout = 5000, token, clientName){
  return new Promise(async (resolve, reject) => {
  let child = this.hasChild(jobId);
  if(!child || child.killed) return resolve(child && child.killed);
  killAsync(child, signal, timeout)
        .then(res => {
          return this.multi()
              .hmset(this.toKey(jobId), {'killedOn': Date.now(), 'killedReason': 'killed', "killPending": false})
              .exec()
        })
        .then(() => {
          debuglog('jobId ' + jobId + ' killed via local event')
          return this.publish('killed:success', {jobId: jobId, pid: child.pid});
        })
        .catch(err => {
          return this.publish('killed:error', {jobId: jobId, pid: child.pid, error: err.message});
        })
        .finally(() => {
          this.client.hset(this.toKey(jobId), 'killPending' , false);
          resolve(true)
        })
  })
}
...
Queue.prototype._setupQueueEventListeners = function() {
  ...
  */
  ...
  const killedKey = this.keys.killed;
  ...
  const pmessageHandler = (pattern, channel, message) => {
    ...
    switch (key) {
      ...
      //       let child = Object.values(this.childPool.retained).find(c=>{
      //         return c && c.jobId && c.jobId === data.jobId
      //       })
      //       if(child && child.jobId === data.jobId){
      //         this.getJobFromId(data.jobId).then(job=>{
      //           killAsync(child, data.signal || "SIGKILL", data.timeout || 5000).then(()=>{
      //             job.log('killed')
      //             return this.publish('killed:success', {jobId: data.jobId, pid: child.pid});
      //           }).catch((err)=>{
      //             return this.publish('killed:error', {jobId: data.jobId, pid: child.pid, error: err.message});
      //           })
      //
      //         })
      //       }
  ...
  const messageHandler = (channel, message) => {
    ...
    switch (key) {
      ...
      case drainedKey:
        ...
        break;
      case this.toKey('killed:success'):
        let successVal = JSON.parse(message);
        this.emit('global:killed:success', successVal.jobId, successVal.pid);
        break;

      case this.toKey('killed:error'):
        let errorVal = JSON.parse(message);
        this.emit('global:killed:error', errorVal.jobId, errorVal.pid, errorVal.error);
        break;

      case killedKey:
        const {jobId, signal, timeout, token, clientName} = JSON.parse(message);
        this.killJobById(jobId,signal,timeout, token, clientName).catch(err=>{
          debuglog('killJobById error' + err.toString())
        });
        break;
    }
  };
  ...
  if (
    ...
    if (!this.registeredEvents[_eventName]) {
      return utils
        ...
        .then(() => {
          const channel = this.toKey(_eventName);
          //if (['active', 'waiting', 'stalled', 'killed'].indexOf(_eventName) !== -1) {
          if (['active', 'waiting', 'stalled'].indexOf(_eventName) !== -1) {
            return (this.registeredEvents[_eventName] = this.eclient.psubscribe(
              channel + '*'
            ));
